...
def main():
    ...
    spark = SparkSession.builder.appName("AWSGlueCampaignData").getOrCreate()

    # Instancie sua classe
    campaign = Campaign(account_id, token, api_url)

    # Use a classe para obter dados
    active_campaigns_data = campaign.get_insights_active_campaigns()

    # Converta os dados para DataFrame do Spark
    df = spark.createDataFrame(active_campaigns_data, schema)

    # Salve os dados em formato Parquet no S3
    df.write.mode("append").partitionBy("created_at").parquet(s3_path_data_lake)

    # Configure a conexão JDBC e salve os dados no PostgreSQL
    df.write \
        .format("jdbc") \
        .option("url", pg_url) \
